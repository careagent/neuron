...
must_haves:
  ...
  artifacts:
    ...
    - path: "src/registration/heartbeat.ts"
      provides: "Heartbeat loop with exponential backoff"
...
1. Create `src/registration/heartbeat.ts`:
   - Export `HeartbeatManager` class:
     - Constructor: `(client: AxonClient, stateStore: RegistrationStateStore, intervalMs: number, backoffCeilingMs: number, onStatusChange?: (status: 'healthy' | 'degraded') => void)`
     - `start(): void` — set `isRunning = true`, schedule first beat at `intervalMs`
     ...
     - Private `async beat()`:
       ...
       3. If no state or status !== 'registered', schedule next at intervalMs and return
       4. Try: call `client.updateEndpoint(registrationId, { neuron_endpoint_url })`, reset `attempt = 0`, update stateStore heartbeat timestamp, call `onStatusChange?.('healthy')` if transitioning from degraded, schedule next at `intervalMs`
   ...
   - Per locked decision: 60-second interval (passed in, but fixed by caller). Backoff ceiling configurable (default 5 min from config).
...
2. Create `src/registration/service.ts`:
   - Export `AxonRegistrationService` class:
     ...
     - `async start(): Promise<void>`:
       ...
       7. Start heartbeat with `config.heartbeat.intervalMs` and `config.axon.backoffCeilingMs`
...
3. Create `src/registration/index.ts`:
   - Barrel export: `AxonRegistrationService`, `AxonClient`, `AxonError`, `RegistrationStateStore`, `HeartbeatManager`
...
4. Create `src/registration/registration.test.ts`:
   ...
   - HeartbeatManager tests:
     - Calls updateEndpoint on interval (use vi.useFakeTimers)
  ...
  <done>
    - HeartbeatManager maintains reachable status with 60s interval, enters exponential backoff (base 5s, full jitter, configurable ceiling) on failure, auto-recovers
