...
must_haves:
  truths:
    - "Registration state tables exist in SQLite after migration v2 runs"
    - "NeuronConfig schema includes an axon section with registryUrl, endpointUrl, backoffCeilingMs"
    - "Mock Axon server starts as a standalone process and responds to registration API calls"
  artifacts:
...
1. Create `test/mock-axon/server.ts`:
   ...
   - Use in-memory `Map` for state (fresh per run per Claude's Discretion — optimize for test reliability)
